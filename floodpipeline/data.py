...
class StationDataUnit:
    ...
    def __init__(self, **kwargs):
        ...
        #self.basin_id: str = kwargs.get("basin_id") # associated basins/districts
        
class BasinDataUnit:
    """Base class for basin data units"""
...
class RainfallBasinDataUnit(BasinDataUnit):
    """Rainfall forecast data unit - basin"""
...
class ForecastBasinDataUnit(BasinDataUnit):
    """Flood forecast data unit - station"""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.lead_time: int = kwargs.get("lead_time")
        self.forecasts: List[RainForecast] = kwargs.get("forecasts", None)
        self.triggered: bool = kwargs.get("triggered", None)  # triggered or not
        self.return_period: float = kwargs.get("return_period", None)  # return period
        self.alert_class: str = kwargs.get("alert_class", None)  # alert class
...
class ThresholdBasinDataUnit(BasinDataUnit):
...
class BasinDataSet:
    """Base class for basin data sets"""

    def __init__(
        self,
        country: str = None,
        timestamp: datetime = datetime.now(),
        data_units: List[BasinDataUnit] = None,
    ):
        self.country = country
        self.timestamp = timestamp
    ...
    def get_data_unit(self, hybasid: str, lead_time: float = None) -> BasinDataUnit:
        if lead_time is not None:
            bdu = next((x for x in self.data_units if x.hybasid == hybasid and x.lead_time == lead_time), None)
        else:
            bdu = next((x for x in self.data_units if x.hybasid == hybasid), None)
        if not bdu:
            raise ValueError(f"Data unit with basin_id {hybasid} not found")
        return bdu

    def upsert_data_unit(self, data_unit: BasinDataUnit):
        existing = next((i for i,x in enumerate(self.data_units)
                         if x.hybasid == data_unit.hybasid and getattr(x, 'lead_time', None) == getattr(data_unit, 'lead_time', None)), None)
        if existing is None:
            self.data_units.append(data_unit)
        else:
            self.data_units[existing] = data_unit
...
class PipelineDataSets:
    ...
    def __init__(self, country: str, settings: Settings):
        ...
        self.threshold_basin = BasinDataSet(
            country=self.country,
            timestamp=datetime.today()
        )
